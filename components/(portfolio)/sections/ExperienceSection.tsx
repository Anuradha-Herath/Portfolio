...
const FeaturedExperience: React.FC<{
  ...
  isMobile?: boolean;
}> = ({ experience, formatDate, isMobile = false }) => {
  ...
  const techVariants = {
    ...
    visible: (index: number) => ({
      ...
      scale: 1,
      transition: {
        delay: isMobile ? 0.3 + index * 0.05 : 0.8 + index * 0.1,
        ...
        <Card className="group relative overflow-hidden bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200/60 dark:border-slate-700/60 shadow-xl hover:shadow-2xl transition-all duration-300 rounded-2xl sm:rounded-3xl">
          ...
          <CardContent className={`relative ${isMobile ? 'p-4 sm:p-6' : 'p-6 sm:p-8 lg:p-10'}`}>
            ...
            <motion.div variants={itemVariants} className={`text-center ${isMobile ? 'mb-4' : 'mb-6 sm:mb-8'}`}>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${isMobile ? 'text-xs' : 'text-sm'} font-medium`}>{experience.location}</span>
                </div>
            ...
            >
              ...
              <span className={`${isMobile ? 'text-xs' : 'text-sm'}`}>Professional Experience</span>
            </motion.div>
          </CardContent>
        </Card>
      </div>
...
export function ExperienceSection() {
  ...
  return (
    ...
    >
      {/* Enhanced Background with floating elements - simplified for mobile */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className={`text-center ${isMobile ? 'mb-8' : 'mb-12 sm:mb-16 lg:mb-20'}`}
        >
          <Heading
            level={2}
